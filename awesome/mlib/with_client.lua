...
_wm.relative_screen_focus = function(offset) return function() awful.screen.focus_relative(offset) end end
_wm.relative_client_focus = function(offset) return function() awful.client.focus.byidx(offset)    end end
...
_wm.mv_bottom_left = function() 
  return function()
    if client.focus.floating or awful.layout.getname() == 'floating' and not client.focus.maximized then
      ...
      awful.placement.bottom_left(client.focus, {honor_padding=true, honor_workarea=true, margins = _mv_margins, offset = _mv_offset})
    else
...
_wm.mv_bottom_right = function() 
  return function()
    if client.focus.floating or awful.layout.getname() == 'floating' and not client.focus.maximized then
      awful.placement.scale(client.focus, {to_percent= 0.46})
      awful.placement.bottom_right(client.focus, {honor_padding=true, honor_workarea=true, margins = _mv_margins, offset = {x = -10, y = 5}})
    else 
      awful.client.swap.byidx( -1) 
    end 
  end
...
_wm.mv_top_left = function () 
  return function()
    if client.focus.floating or awful.layout.getname() == 'floating' and not client.focus.maximized then
      awful.placement.scale(client.focus, {to_percent= 0.46})
      awful.placement.top_left(client.focus, {honor_padding=true, honor_workarea=true, margins = _mv_margins, offset = _mv_offset})
    else
...
_wm.mv_top_right = function () 
  return function()
    if client.focus.floating or awful.layout.getname() == 'floating' and not client.focus.maximized then
      awful.placement.scale(client.focus, {to_percent= 0.46})
      awful.placement.top_right(client.focus, {honor_padding=true, honor_workarea=true, margins = _mv_margins, offset = {x = -10, y = 5}})
    else
      awful.tag.incnmaster(-1, nil, true) 
    end
  end
...
_wm.dech = function() 
  return function() --[[ k ]]
    if client.focus.floating or awful.layout.getname() == 'floating' then
      ...
      mdbg_t(client.focus.screen.geometry, 'sgeo') mdbg_t(swrk,'swrk') mdbg_t(cgeo,'cgeo') mdbg_t(ugeo, 'ugeo')
...
_wm.decw = function(...) 
  return function(...) --[[ h ]]
    if client.focus.floating or awful.layout.getname() == 'floating' then
      local cgeo = client.focus:geometry()
      client.focus:geometry({x = cgeo.x < 0 and 0 or cgeo.x, y = cgeo.y, width = ( cgeo.width - 30 ), height = cgeo.height })
    else
      awful.tag.incmwfact(-0.01)
    end
  end 
...
_wm.incncol = function() return function() awful.tag.incncol( 1, nil  , true) end end --[[ h ]]
_wm.decncol = function() return function() awful.tag.incncol(-1, nil  , true) end end --[[ l ]]
