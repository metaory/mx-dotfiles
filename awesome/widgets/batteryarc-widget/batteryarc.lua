-------------------------------------------------
-- Battery Arc Widget for Awesome Window Manager
-- Shows the battery level of the laptop
-- More details could be found here:
-- https://github.com/streetturtle/awesome-wm-widgets/tree/master/batteryarc-widget

-- @author Pavel Makhov
-- @copyright 2020 Pavel Makhov
-------------------------------------------------

local awful = require("awful")
local thm = require("beautiful")
local naughty = require("naughty")
local wibox = require("wibox")
local watch = require("awful.widget.watch")

local HOME = os.getenv("HOME")
local WIDGET_DIR = HOME .. '/.config/awesome/widgets/batteryarc-widget'

local batteryarc_widget = {}

local function worker(user_args)

  local args = user_args or {}

  local font = args.font or thm.arc_font or thm.font
  local arc_thickness = args.arc_thickness or thm.arc_thickness or 4
  local show_current_level = args.show_current_level or true
  local size = args.size or thm.arc_size or 28
  local timeout = args.timeout or 10
  local notif_backoff = 300
  local cable_backoff = 120
  local show_notification_mode = args.show_notification_mode or 'on_hover' -- on_hover / on_click
  local last_charging_value = 0 
  -- xcritical
  -- xwarning

  local crt_lvl = { thrsh = 20 , clr = thm.red_l    or '#e53935' }
  local low_lvl = { thrsh = 30 , clr = thm.red_d    or '#e53935' }
  local med_lvl = { thrsh = 40 , clr = thm.yellow_d or '#e53935' }

  -- local low_level_color = args.low_level_color or thm.red_d or '#e53935'
  -- local medium_level_color = args.medium_level_color or thm.red_x or '#e53935'

  local main_color = args.main_color or thm.xhigh_1
  local bg_color = args.bg_color or thm.fg_dark or '#ffffff11'
  -- local med_lvl.clr = '#f39c12a8' -- args.med_lvl.clr or thm.yellow_d or '#c0ca33'
  local charging_color = args.charging_color or thm.green_d or '#43a047'

  local warning_msg_title = args.warning_msg_title or 'Houston, we have a problem'
  local warning_msg_text = args.warning_msg_text or 'Battery is dying'
  local warning_msg_cabl = args.warning_msg_cabl or 'Cable Malfunction'
  local warning_msg_position = args.warning_msg_position or 'bottom_right'
  local warning_msg_icon =  args.warning_msg_icon or WIDGET_DIR .. '/spaceman.jpg'
  local enable_battery_warning = args.enable_battery_warning or true
  -- local enable_battery_warning = args.enable_battery_warning
  -- i f enable_battery_warning == nil then enable_battery_warning = true end

  local text = wibox.widget {
    font = font,
    align = 'center',
    valign = 'center',
    widget = wibox.widget.textbox
  }

  local text_with_background = wibox.container.background(text)
  -- text_with_background.opacity = 0.2

  batteryarc_widget = wibox.widget {
    text_with_background,
    max_value = 100,
    rounded_edge = true,
    thickness = arc_thickness,
    start_angle = 4.71238898, -- 2pi*3/4
    forced_height = size,
    forced_width = size,
    bg = bg_color,
    -- paddings = 2,
    widget = wibox.container.arcchart
  }

  local last_battery_check = os.time()
  local last_cable_check = os.time()

  --[[ Show warning notification ]]
  local function show_battery_warning(cur, msg)
    naughty.notify {
      icon = warning_msg_icon,
      icon_size = 100,
      text = (msg or warning_msg_text)..'<b>'..tostring(cur)..'</b>',
      title = warning_msg_title,
      timeout = 25, -- show the warning for a longer time
      hover_timeout = 0.5,
      position = warning_msg_position,
      bg = thm.bg_normal,
      fg = low_lvl.clr,
      width = 400,
    }
  end

  local function update_widget(widget, stdout)
    local charge = 0
    local status
    for s in stdout:gmatch("[^\r\n]+") do
      local cur_status, charge_str, _ = string.match(s, '.+: (%a+), (%d?%d?%d)%%,?(.*)')
      if cur_status ~= nil and charge_str ~=nil then
        local cur_charge = tonumber(charge_str)
        if cur_charge > charge then
          status = cur_status
          charge = cur_charge
        end
      end
    end

    widget.value = charge


    if show_current_level == true then
      --- i f battery is fully charged (100) there is not enough place for three digits, so we don't show any text
      text.text = charge == 100
      and ''
      or string.format('%d', charge)
    else
      text.text = ''
    end

    if status == 'Charging' then
      text_with_background.fg = charging_color
      text_with_background.bg = thm.bg_normal
    else
      -- text_with_background.bg = '#00000000'
      text_with_background.bg = thm.bg_focus
      text_with_background.fg = thm.xhigh_1
    end

    if charge < crt_lvl.thrsh then
      mdbg_p(' -- # 1 # <<')
      widget.colors = { crt_lvl.clr }
      text_with_background.bg = crt_lvl.clr
      text_with_background.fg = '#000000'
      if enable_battery_warning and status ~= 'Charging' and os.difftime(os.time(), last_battery_check) > notif_backoff then
        last_battery_check = os.time()
        show_battery_warning(charge)
      end
      if status == 'Charging' then
        text_with_background.bg = charging_color
      end
    elseif charge < low_lvl.thrsh then
      mdbg_p(' -- # 2 # <<')
      widget.colors = { low_lvl.clr }
      -- text_with_background.bg = low_lvl.clr
      -- text_with_background.fg = '#000000'
    elseif charge < med_lvl.thrsh then
      mdbg_p(' -- # 3 # <<')
      widget.colors = { med_lvl.clr }
      -- text_with_background.fg = low_lvl.clr
      -- text_with_background.bg = '#000000'
    else
      mdbg_p(' -- # 4 # <<')
      widget.colors = { main_color }
      -- text_with_background.fg = low_lvl.clr
      --
      if status == 'Charging' then
        -- text_with_background.bg = charging_color
        text_with_background.bg = thm.bg_normal
        widget.colors = { charging_color }
      end
    end

    mdbg_p('>>>>')
    mdbg_t({st=status,lv=last_charging_value,cr=charge},'BT')
    mdbg_p('<<<<')

    -- i f os.difftime(os.time(), last_cable_check) > notif_backoff then

      if status == 'Charging' and os.difftime(os.time(), last_cable_check) > cable_backoff then
        mdbg_p('1 <')
        last_cable_check = os.time()
        if status == 'Charging' and charge < last_charging_value then
          mdbg_p('2 <<')
          show_battery_warning(charge, warning_msg_cabl .. string.format('\ndown %s points since last check', tostring(last_charging_value-charge)))
          text_with_background.bg = thm.xhigh_2
        end
      end

      last_charging_value = charge
    end

    watch("acpi", timeout, update_widget, batteryarc_widget)

    -- Popup with battery info
    local notification
    local function show_battery_status()
      awful.spawn.easy_async([[bash -c 'acpi']],
        function(stdout, _, _, _)
          naughty.destroy(notification)
          notification = naughty.notify {
            text = stdout,
            title = "Battery status",
            timeout = 5,
            width = 200,
          }
        end)
      end

      if show_notification_mode == 'on_hover' then
        batteryarc_widget:connect_signal("mouse::enter", function() show_battery_status() end)
        batteryarc_widget:connect_signal("mouse::leave", function() naughty.destroy(notification) end)
      elseif show_notification_mode == 'on_click' then
        batteryarc_widget:connect_signal('button::press', function(_, _, _, button)
          if (button == 1) then show_battery_status() end
        end)
      end

      return batteryarc_widget

    end

    return setmetatable(batteryarc_widget, { __call = function(_, ...)
      return worker(...)
    end })
