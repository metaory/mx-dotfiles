...
mmenus.grd_menu = function()
  ...
  for c in awful.client.iterate(etc_rule) do
	...
	for _, c in pairs(client.get()) do
		...
		elseif
			awful.rules.match_any(c, {
				...
				class = { "kitty", "M_SYS_LOG" },
			})
		then
			item[3] = thm.trm_icon
			items[2][2][#items[2][2] + 1] = item
		elseif awful.rules.match_any(c, { class = { "Chromium" } }, { instance = { "chromium" } }) then
			item[3] = thm.web_icon
			items[3][2][#items[3][2] + 1] = item
		else
			item[3] = thm.etc_icon
			if awful.rules.match(c, { class = "Spotify" }) then
				item[3] = thm.spotify_icon
			end
			if awful.rules.match(c, { class = "Steam" }) then
				item[3] = thm.steam_icon
			end
			if awful.rules.match(c, { class = "Slack" }) then
				item[3] = thm.slack_icon
			end
			items[#items + 1] = item
		end
	end
	...
	if #items[1][2] == 0 then
		items[1].theme = { submenu_icon = thm.menu_submenu_icon_empty, fg_normal = thm.fg_dark, fg_focus = thm.fg_dark }
	end
	if #items[2][2] == 0 then
		items[2].theme = { submenu_icon = thm.menu_submenu_icon_empty, fg_normal = thm.fg_dark, fg_focus = thm.fg_dark }
	end
	if #items[3][2] == 0 then
		items[3].theme = { submenu_icon = thm.menu_submenu_icon_empty, fg_normal = thm.fg_dark, fg_focus = thm.fg_dark }
	end
